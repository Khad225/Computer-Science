...
public class SMTPInteraction {
	...
	connection = new Socket(mailmessage.DestHost, mailmessage.DestHostPort);/* Fill in */
    ...
    public void send(EmailMessage mailmessage) throws IOException {
		...
		sendCommand(mailmessage.toString() + CRLF + "." + CRLF, 250);
	...
	/* Check that the server's reply code is the same as the parameter
	   rc. If not, throw an IOException. */
		...
		if(!text.startsWith(Integer.toString(rc))) {
